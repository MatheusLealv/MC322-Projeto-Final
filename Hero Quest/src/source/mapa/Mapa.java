...
import source.items.Tesouro;
import source.monstro.Esqueleto;
import source.monstro.EsqueletoMago;
import source.monstro.Goblin;
import source.monstro.Monstro;
import source.utilities.Pair;
...
public class Mapa {
	...
	public void generateMap(Unit hero) {
		for(int i = 0 ; i < N ; i ++) {
	...
	public void trocaCel(int a , int b , int c , int d) {
		Celula A = grid[a][b];
		Celula B = grid[c][d];
		grid[a][b] = B;
		grid[c][d] = A;
